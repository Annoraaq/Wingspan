...
/**
 * Description of PMemoryConstraintProcessor
 *
 * @author J. Baum
 */
class PMemoryConstraintProcessor implements PConstraintProcessor {
    ...
    private function order(PClassConstraint $constraint, PResultSet $resultSet) {
        ...
        if ($constraint->getOrder() == PClassConstraint::ORDER_DESC) {
            usort($resArr,
             array('\ParrotDb\Query\Constraint\PMemoryConstraintProcessor',
                'cmpDesc'));
        } else {
            usort($resArr,
             array('\ParrotDb\Query\Constraint\PMemoryConstraintProcessor',
